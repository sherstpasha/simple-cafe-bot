...
def _build_messages_with_exact_prompt(user_text: str, menu: dict) -> list[dict]:
    ...
    system_instructions = (
        ...
        "  • -1 — не указано\n\n"
        "– Определи staff:\n"
        "  • 1 — заказ явно для сотрудника (фразы вроде \"для сотрудника\", \"сотруднику\")\n"
        ...
        '  "staff":0\n'
        "}\n\n"
        '- Запрос: "сделай, пожалуйста, американо и круассан для сотрудника"\n'
        "{\n"
        '  "it":[\n'
        '    {"n":"Американо","q":1,"a":[]},\n'
        '    {"n":"Десерты: Круассан","q":1,"a":[]}\n'
        "  ],\n"
        '  "pay":-1,\n'
        '  "staff":1\n'
        "}\n\n"
        ...
        "{\n"
        '  "it":[\n'
        '    {"n":"Американо с молоком доя олега перевод","q":1,"a":["Молоко"]}\n'
        "  ],\n"
        '  "pay":1,\n'
        '  "staff":0\n'
        "}\n\n"
...
async def parse_order_from_text(
    ...
    try:
        ...
        pay_code = -1
    try:
        staff_flag = int(result.get("staff", 0))
    except Exception:
        staff_flag = 0
    is_staff = 1 if staff_flag else 0
    return {"it": raw_items, "pay": pay_code, "staff": is_staff}
